...
render_quality_report_index <- function(
  ...
  master_files  <- files[grepl("^bmf_unified(_geocoded)?_quality_report\\.html$", files)]
  ...
  master_section <- section_html(
    "Unified BMF",
    lapply(master_files, function(f) li(f, if (grepl("geocoded", f))
      "Geocoded Unified BMF Quality Report" else "Unified BMF Quality Report")),
    "Not yet generated."
  )
