...
revision <- stringr::str_match(html, "Instructions for Form 1023 \\((\\d{2}/\\d{4})\\)")[, 2]
...
raw <- tibble::tibble(
  ntee_code   = matches[, 2],
  description = matches[, 3] |> unescape() |> stringr::str_squish()
)
...
conflicting <- raw |>
  dplyr::distinct(ntee_code, description) |>
  dplyr::count(ntee_code) |>
  dplyr::filter(n > 1)
...
stopifnot(
  all(stringr::str_detect(raw$ntee_code, "^[A-Z][0-9][0-9A-Z]$")),
  all(nchar(raw$description) >= 4),
  !any(stringr::str_detect(raw$description, "[<>]"))
)

irs <- raw |>
  dplyr::distinct(ntee_code, .keep_all = TRUE) |>
  dplyr::arrange(ntee_code) |>
  dplyr::mutate(irs_source = "Instructions for Form 1023, Appendix D",
...
if (file.exists(OUT_CSV)) {
  previous <- readr::read_csv(OUT_CSV, show_col_types = FALSE)
...
ours <- openxlsx::read.xlsx(LOOKUP, sheet = "ntee_code") |>
  tibble::as_tibble() |>
  dplyr::mutate(ntee_code = stringr::str_trim(ntee_code))

missing_from_lookup <- irs |> dplyr::anti_join(ours, by = "ntee_code")
not_in_irs          <- ours |> dplyr::anti_join(irs, by = "ntee_code") |> dplyr::filter(ntee_code != "Z99")

cat("\nIRS codes missing from the lookup sheet:", nrow(missing_from_lookup), "\n")
if (nrow(missing_from_lookup) > 0) print(missing_from_lookup |> dplyr::select(ntee_code, description), n = Inf)

cat("\nLookup codes the IRS no longer lists (kept for old records; no action unless they cause problems):",
    nrow(not_in_irs), "\n")
if (nrow(not_in_irs) > 0) print(not_in_irs |> dplyr::select(ntee_code, ntee_code_definition), n = Inf)
