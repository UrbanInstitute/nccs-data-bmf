...
- `s3://nccsdata/unified/bmf/ein-index/latest/{prefix}.json` — EIN index (ADR 0050): one gzip JSON shard per three-digit EIN prefix with identity, NTEE and status columns, so the website can look an organization up by EIN in the browser. Built by `R/build_ein_index.R`, run via `R/run_ein_index.R`, rebuilt after every geocoded merge.
