...
|--------|------|-------------|
| `ein_raw` | character | Source EIN as ingested (non-digits removed, **not** zero-padded) — lossy bare-integer surface (e.g. `000000004` → `4`); use `ein`/`ein_prefixed`/`EIN2` to join |
| `ein` | character | Formatted as XX-XXXXXXX (canonical published key) |
...
`legacy_mode = TRUE` (set automatically by `run_legacy_pipeline.R`)
relaxes the >4-char NTEE length validator from a hard stop to a
warning, and applies the vendored
`data/lookup/ntee_legacy_5char_lookup.csv` crosswalk so pre-2003
5-character NCCS NTEE codes (e.g. `A0120`) resolve to canonical
...
`nteev2_code` is the cleaned NTEE-CC code with the NTEE-V2 x00 rule
applied (ADR 0048: specialty/common codes 01-19 collapse to `x00`, the
organization type living in `nteev2_org_type`), falling back to `Z99` only
...
| `nteev2_subsector_definition` | character | Human-readable NTEEv2 subsector name |
| `nteev2_org_type` | character | NTEEV2 organization type |


**Sentinel Values:**

Several NTEE output columns use sentinel string values to represent missing, invalid, or inapplicable data. These values are intentionally distinct:

| Column | Value | Meaning |
|--------|-------|---------|
| `ntee_code_clean` | `UNDEFINED` | Source NTEE code is missing or empty |
| `ntee_code_clean` | `INVALID` | Source NTEE code is malformed (no leading letter) or not found in the lookup table |
| `ntee_code_major_group` | `Unknown` | Valid Z-prefix NTEE code — the lookup table maps Z codes to "Unknown" (IRS code Z99 = "unclassifiable organization") |
| `ntee_code_major_group` | `UNDEFINED` | Source NTEE code is missing/empty or invalid (propagated from `ntee_code_clean`) |
...
**Validation behaviour**: rows with non-6-character `TAX_PERIOD`
values emit a warning rather than aborting the pipeline; downstream
`lubridate::ymd()` returns NA for unparseable values, `tax_period_is_missing`
flags them, and the sentinel-date fill takes over. Mirrors the pattern in
...
```r
transform_accounting_period(dt, input_col = "ACCT_PD")
```
