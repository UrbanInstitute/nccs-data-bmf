...
| `BMF_S3_GEOCODING_PREFIX` | character | `"geocoding/bmf/"` | S3 prefix for geocoded outputs |
| `BMF_S3_LEGACY_PREFIX` | character | `"legacy/bmf/"` | S3 prefix for raw legacy 501CX-NONPROFIT-PX files |
| `BMF_S3_LEGACY_INTERMEDIATE_PREFIX` | character | `"intermediate/bmf-legacy/"` | S3 prefix for legacy intermediate outputs |
| `BMF_S3_LEGACY_PROCESSED_PREFIX` | character | `"processed/bmf-legacy/"` | S3 prefix for legacy processed outputs |
| `GEOCODER_BATCH_SIZE` | integer | `900000` | Max rows per geocoder batch (under 1M limit) |
...
# Upload geocoded BMF (parquet, CSV, quality report, data dictionary)
upload_geocoded_bmf(parquet_path, csv_path, quality_report_path, dictionary_path, year, month)

# Upload S3 bucket README from data/s3-readme/README.md
upload_s3_readme()
```
...
| `bmf_YYYY_MM_geocoded.parquet` | `s3://nccsdata/geocoding/bmf/YYYY_MM/merged/` | Geocoded BMF (parquet) |
| `bmf_YYYY_MM_geocoded.csv` | `s3://nccsdata/geocoding/bmf/YYYY_MM/merged/` | Geocoded BMF (CSV) |
| `bmf_YYYY_MM_geocoding_quality_report.json` | `s3://nccsdata/geocoding/bmf/YYYY_MM/merged/` | Geocoding quality metrics |
| `bmf_YYYY_MM_geocoding_data_dictionary.csv` | `s3://nccsdata/geocoding/bmf/YYYY_MM/merged/` | Geocoding column metadata |
| `README.md` | `s3://nccsdata/` | Bucket documentation (from `data/s3-readme/README.md`) |
...
│   │   ├── geocoding_checks.R     # Geocoding quality validation
│   │   └── quality_report_template.qmd  # HTML/PDF report template
│   ├── run_geocoding.R            # Geocoding workflow orchestrator
...
│   │
│   ├── s3-readme/                  # Version-controlled S3 bucket README
│   │   └── README.md              # Uploaded to s3://nccsdata/README.md
│   │
│   ├── geocoding/                  # Geocoding workflow data (gitignored)
│   │   └── YYYY_MM/
│   │       ├── input/             # Address batches for geocoder
│   │       ├── output/            # Raw geocoder results
│   │       └── merged/            # Final geocoded BMF
│   │
